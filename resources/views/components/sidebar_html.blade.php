...
<nav class="sidebar fixed-left" id="sidebar">
    ...
    <div class="sidebar_list overflow-auto">

        @forelse ($categorias as $indice => $categoria)
        <div class="sidebar_list__item btn btn-primary" id="buttonCategoria">
            <div class="d-none" id="idCategoria">{{ $categoria['id'] }}</div>
            <span class="list_item__name">{{ $categoria['titulo_categoria'] }}</span>
            <div class="list_item__actions">
                <a href="{{ route('categoria.buscar', ['id' => $categoria['id']]) }}" class="list_item__action__link btn btn-warning"><i class="bi bi-pencil"></i></a>
                <a href="{{ route('categoria.deletar', ['id' => $categoria['id']]) }}" class="list_item__action__link btn btn-danger"><i class="bi bi-trash3"></i></a>
            </div>
        </div>
        @empty
